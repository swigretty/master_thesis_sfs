...
\begin{frame}[fragile]
	\frametitle{Population Level Time Series Characteristics}
	Time Series characteristics from David
...
\begin{frame}
	...
	\begin{itemize}
		\item Within subject weekly BP sample variance (): \\
		(This is also biased since we assume iid data and since we have
		non-uniform sampling)
		\item measurement noise (Gaussian White noise with var=62 mmHg)
		\item cyclic pattern amplitude (5 mmHg or a range ?)
		\item cyclic pattern period (=24h)
		\item AR(1) component
		\item long term trend
	...
	\begin{block}{Block Title}
		Do we want to simulate the whole population (i.e. sample from the population level distribution
		over data density, meas noise, cycle amplitude etc.) ? \\
		If so do we want to put priors also on GP hyperparameters e.g. meas noise, cycle amplitude based on
		the population level distribution ? \\
		Or do we just look at ranges to tell  when methods fail?
	\end{block}
...
\begin{frame}
	\frametitle{Extract Time Series Characteristics of Interest and Evaluate}
