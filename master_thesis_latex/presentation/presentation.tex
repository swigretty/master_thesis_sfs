...
\begin{frame}
	\frametitle{Simulation of BP Time Series}
	Simulation of true BP signal and measurements:
		\begin{itemize}
			\item $X=\{x_1, \dots x_n\}$: The input time points of interest. We are interested in a time span of 1 week
			with 10 BP values per hour.
			\item $f_X := f([x_1 \dots x_n])$: The true BP signal $f(x)$ drawn from $GP(0, k_{true}(x,x'))$ evaluated at inputs $X$.
			\item $y_X := y([x_1 \dots x_n])$: The noisy BP measurements. $y(x)= f(x) + \epsilon$ with $\epsilon \sim N(0, \sigma_n^2)$
		\end{itemize}
	\bigskip
	Based on subsampling scheme choose $X_{train} = \{x_1, \dots x_m\} \subset X$ and $X_{test} = X \setminus X_{train}$.

	\bigskip

	Fit GP regression model to training data $y_{train} := y_{X_{train}}$:
			\begin{itemize}
				\item $k_{fit}$: The fitted kernel function with hyperparameters $\theta$ that maximizes the marginal likelihood
				$p(y_{train}| \theta)$
				\item $p(f_X| y_{train}, k_{fit}) = N(\bar{\mu}_X, \bar{\Sigma}_X)$:
				The posterior (predictive) probability density of $f_X$. The posterior mean vecor
				$\bar{\mu}_X$ contains the point estimates.
			\end{itemize}
	\bigskip
	Evaluate predictive probabilities $p(f_{X_{train}}| y_{train})$,
		$p(f_{X_{test}}| y_{train})$ and $p(f_{X}| y_{train})$
			\begin{itemize}
				\item $\bar{\mu}_{X_{train}} != f(X_{train})$ due to the measurement error. If
				$f_{train} := f(X_{train})$ was known: $p(f_{train}| f_{train}, GP_{fit}) = N(\bar{\mu}_{X_{train}},
				\bar{\Sigma}_{X_{train}})$
				with $\bar{\mu}_{X_{train}} = f_{train}$ and $\bar{\Sigma}_{X_{train}} = {\displaystyle O}$
			\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item True: mean($f_{X}$)
			\item GP Regression: mean($\bar{\mu}_{X}$) with standard error
			$1 / n * \sqrt {\sum_{i=1}^{i=n}\sum_{j=1}^{j=n} \bar{\Sigma}_{X, (i,j)}}$ or sample from posterior
