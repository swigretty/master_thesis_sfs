...
\begin{frame}
	\frametitle{Bayesian Inference}
	\begin{figure}
			\includegraphics[width=0.4\linewidth]{bayesian_learining}
			\caption{X2: RV representing the value $f^{\ast} := f(x^\ast)$ we want to predict,
				X1: RV representing observed value $Y_1 := Y(X_1)$. We observe $y_1$ (dashed line)}
	\end{figure}
