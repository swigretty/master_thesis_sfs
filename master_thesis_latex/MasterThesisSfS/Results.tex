...
\caption[Impact of Downsampling Factor]{Impact of Downsampling Factor:
    The estimated BP values $\hat{f}(x)$(black)
    and CIs (gray area) based on measurements (red dots) and using different methods.
    Measurments have been produced using a
    downsampling factor of 20 (left panels) and 2.5 (right panels).
    The true BP values, $f(x)$, are shown by the red dotted line.}
...
\caption[Seasonal Samping and Downsampling Factor]{Seasonal Samping and Downsampling Factor:
    The estimated BP values $\hat{f}(x)$(black)
    and CIs (gray area) based on measurements (red dots) and using different methods.
    The measurements have been produced by extreme seasonal sampling and
...
\caption[Dominant Cyclic Component vs. Dominant AR Component]{
    ...
    Estimated BP values $\hat{f}(x)$(black)
    and CIs (gray area) based on measurements (red dots) and using different methods.
    Measurements have been uniformly sampled with a downsampling factor of 5.
    The true BP values, $f(x)$ are shown by the red dotted line.
    The right  panels show the BP values produced by a process with prominent
    AR component while in the left panels the cyclic component is prominent.
 }
...
Boostrapping in these conditions leads to very unstable BP estimates,
which is shown in figure \ref{subfig: ex-spline-failure-bootstrap}.
The estimated BP values $\hat{f}(x)$, from one boostrap sample,
comprises very extreme BP values, which will
...
\caption[Smoothing Spline Failure]{
    ...
    for a downsampling factor of 20.
