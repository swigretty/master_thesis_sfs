...
As we have seen in section \ref{sec:gaussian-process-regression}, predictions
in with Gaussian processes always provides a whole predictive distribution not only a point
estimate.
An expression for the predictive (posterior) distribution over $Y^{\ast}:= Y(x^{\ast})$
for some arbitrary input $x^{\ast}$ has been provided in equation \ref{eq:posterior-gp-noise}.
This allows to calculate the probability of the observed value $y^{\ast}$ at an input $x^{\ast}$
under the predictive distribution.

From expression of the predictive distribution in \ref{eq:posterior-gp-noise} we know:

\begin{gather}\label{eq:predictive-dist}
    \log p(Y^{\ast} = y^{\ast}| \mathbf{Y}, X) =
    - \frac{1}{2} \log \bar{\sigma}^2 -
\frac{(y^{\ast} - \bar{\mu})^2}{2 \bar{\sigma}^2} - \frac{1}{2} \log 2 \pi
\end{gather}
where
$\bar{\sigma}^2 = K_{x^{\ast}x^{\ast}} - K_{x^{\ast}X} K_{XX}^{-1} K_{Xx^{\ast}}$
and $\bar{\mu} = K_{x^{\ast}X} K_{XX}^{-1} \mathbf{Y}$.

In contrast, computing the squared error between the true value $y^{\ast}$
and the predictive mean $\bar{\mu}$ does completely ignore the predictive variance
$\bar{\sigma}^2$.
...
The most common approaches to solve this problem are Bayesian model selection and
estimation of the generalization error using cross validation.
A suitable loss function to be used
for cross validation is based on the predictive probablity
of your observations as desribed in section
\ref{sec:performance-assessment}.
Bayesian model selection on the other hand looks for the most probable model given the data.
The rest of this section is dedicated to this approach, which is probably less known.

%Bayesian model selection will be presented in the following subsection.
%Specifically when leaving out training case $i$:
%\begin{gather}\label{eq:loocv}
%    \log p(Y^{\ast} = y^{\ast}| \mathbf{Y}, X) =
%    - \frac{1}{2} \log \bar{\sigma}^2 -
%\frac{(y^{\ast} - \bar{\mu})^2}{2 \bar{\sigma}^2} - \frac{1}{2} \log 2 \pi
%\end{gather}
...
A hierarchical specification of the model helps to assess the probability of
certain model configurations, given the data.
...
However, in the non-parametric setting of Gaussian processes, the parameter $\beta$ do not exist and get replaced by
...
$p(\mathbf{Y}|X, \theta, M_i)$ with respect to the hyperparameters $\theta$.
This approach is equal to assuming uniform distributions over the hyperparameters.
The next subsection will provide more details on chow to calculate and maximize the
