...
Gaussian process regression offers more than just inferring the posterior (predictive) mean
of the response variable; it also provides insights into its posterior (predictive) distribution.
...
In contrast, when computing the sum of squared errors between the true values $y^{\ast}$ and the predictive mean $\bar{\mu}$,
...
Two common approaches for model selection are cross-validation, which utilizes a performance-based loss
function discussed in Section \ref{sec:performance-assessment}, and Bayesian model selection,
...
function values $f^{\ast}$ for a general covariance function of the prior.
