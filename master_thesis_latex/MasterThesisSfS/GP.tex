...
There are many options on how to solve the model selection problem. The most common approaches
are either based on the probability of the model given the data or on the estimation of the
generalization error by cross validation.

The approach involving the calculation of the model probability given the data is called
Bayesian model selection.
One can use a hierarchical specification of the model.
Assuming a parametric model one has
the parameters $\beta$ at the lowest level. In the non parametric setting
one directly models the distribution over the function $f$ instead.
On the next levels are the hyperparameters $theta$ which control the distribution of the
parameters $\beta$ or in the non-parametric setting of the function $f$.
At the highst level is the set of possible model structures $\mathbb{H}_i$.

At the lowest level the posterior over the parameters $\beta$ is given by Bayes rule:

\begin{gather*}
    p(w | \mathbf{Y}, X, \theta, \mathbb{H}_i) = \frac{p( \mathbf{Y}| X, \beta,
        \mathbb{H}_i)p(\beta|\theta, \mathbb{H}_i)}{p(\mathbf{Y}|X, \theta, \mathbb{H}_i)}
\end{gather*}

We call $p(\mathbf{Y}| X, \beta, \mathbb{H}_i)$ likelihood,
$p(\beta|\theta, \mathbb{H}_i)$ the prior
and $p(\mathbf{Y}|X, \theta, \mathbb{H}_i)$ the marginal likelihood.

Similarly, in the non-parametric world of gaussian processes we can
assume a posterior over the function value
$f^{\ast} = f(x^{\ast})$ at some arbitrary input $x^{\ast}$ with $f_X = f(X)$:
\begin{gather*}
    p(f^{\ast} | \mathbf{Y}, X, \theta, \mathbb{H}_i) = \frac{p( \mathbf{Y}| f_X,
        \mathbb{H}_i)p(f| \theta, \mathbb{H}_i)}{p(\mathbf{Y}|X, \theta, \mathbb{H}_i)}
\end{gather*}
...
there is a similar expresson for the posterior over the hyperparameters:
\begin{gather*}
    p(\theta | \mathbf{Y}, X, \mathbb{H}_i) = \frac{p( \mathbf{Y}| X,\mathbb{H}_i, \theta)
        p(\theta| \mathbb{H}_i)}{p(\mathbf{Y}|X, \mathbb{H}_i)}
\end{gather*}

Maximising $p(\theta | \mathbf{Y}, X, \mathbb{H}_i)$ then yields the
optimal hyperparameters. However, when assuming non gaussian priors on $\theta$,
evaluating $p(\theta | \mathbf{Y}, X, \mathbb{H}_i)$ might
be difficult and instead one often just maximizes the marginal likelihood
$p(\mathbf{Y}|X, \theta, \mathbb{H}_i)$ with respect to the hyperparamters $\theta$.
...
These expressions are obtained by marginalization over $\beta$ or the latent function $f$
