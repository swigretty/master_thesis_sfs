...
time points $t_i \in T_0$ with $T_0=\{t_1, t_2 \dots t_n\}$ and $t_1 < t_2 < \dots t_n$, we can write in matrix notation:
\begin{gather*}
...
However, the assumption of uncorrelated errors $E_{t_i}$ is generally violated if $\mathbf{E}$ is a time series and the
estimated covariance matrix $\hat{\Psi}$ is thus biased (\citeauthor{brockwell_introduction_2016}).
...
hence $\mathbf{E} = (E_t: t \in T_0)$ with $T_0 = \{1, 2, \dots  n \}$ and:
...
define the ARMA process assumed for $(E_t)$.
