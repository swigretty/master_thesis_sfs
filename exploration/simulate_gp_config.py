...
_simple_kernels = {k: ConstantKernel(constant_value=v["scale"], constant_value_bounds=v["scale_bounds"]) * v["kernel"](
...
ou_kernels_fixed = {k: (_kernels_fixed["ou"] + v if k != "ou" else v) for k, v in _kernels_fixed.items()}
